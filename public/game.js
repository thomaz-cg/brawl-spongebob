...
// ═══ ISOMETRIC CAMERA ════════════════════════════════════════
const ISO_W=72, ISO_H=36, WALL_H=44;

// Camera tracks player with smooth lerp
let camX=0, camY=0, camTX=0, camTY=0;
const CAM_LERP=0.09;
// Push camera down so player appears lower-center (see more of the map ahead)
const CAM_Y_BIAS = 60;

function wp2s(wx,wy){
  const tc=wx/ISO_W, tr=wy/ISO_W;
  const rawX=(tc-tr)*(ISO_W/2);
  const rawY=(tc+tr)*(ISO_H/2);
  return {
    sx: W/2 + (rawX-camX),
    sy: H/2 + (rawY-camY) + CAM_Y_BIAS,
  };
}
...
function updateCamera(){
  ...
  const me=players[0];
  const tc=me.x/ISO_W, tr=me.y/ISO_W;
  camTX=(tc-tr)*(ISO_W/2);
  camTY=(tc+tr)*(ISO_H/2);
  camX+=(camTX-camX)*CAM_LERP;
  camY+=(camTY-camY)*CAM_LERP;
}
...
// ═══ MAP ═════════════════════════════════════════════════════
function buildMap(){
  // Bigger map so camera scrolling feels natural
  MC=Math.max(18,Math.floor((W*1.2)/ISO_W));
  MR=Math.max(13,Math.floor((H*1.2)/ISO_W));
  tiles=[];blocks=[];
...
function tileAt(px,py){
  const c=Math.floor(px/ISO_W),r=Math.floor(py/ISO_W);
...
function startGame(){
  ...
  // snap camera to player immediately (no lerp slide from 0,0)
  const me=players[0];
  const tc=me.x/ISO_W, tr=me.y/ISO_W;
  camX=camTX=(tc-tr)*(ISO_W/2);
  camY=camTY=(tc+tr)*(ISO_H/2);
...
function aiStep(p){
  ...
  if(!t){
    const cx=MC*ISO_W/2,cy=MR*ISO_W/2;
...
function hitBlock(bx,by){
  const c=Math.floor(bx/ISO_W),r=Math.floor(by/ISO_W);
...
function checkRespawns(){
  for(const p of players)if(!p.alive&&p.respawnAt>0&&frameN>=p.respawnAt){
    p.hp=CHARS[p.char].hp;p.alive=true;p.x=(p.sc+.5)*ISO_W;p.y=(p.sr+.5)*ISO_W;p.respawnAt=0;spawnBoom(p.x,p.y,'spawn');
  }
}
...
function isoTile(col,row,fillTop,fillLeft,fillRight,h){
  h=h||0;
  const {sx,sy}=wp2s(col*ISO_W,row*ISO_W);
...
function drawMap(){
  ...
  // draw tiles back-to-front painter's algorithm (sum = r+c ascending)
  for(let sum=0;sum<MC+MR-1;sum++){
    for(let c=0;c<=sum;c++){
      const r=sum-c;
      if(r<0||r>=MR||c<0||c>=MC)continue;
      // frustum cull: skip tiles too far off screen
      const {sx,sy}=wp2s(c*ISO_W,r*ISO_W);
      if(sx<-ISO_W*2||sx>W+ISO_W*2||sy<-WALL_H-ISO_H*2||sy>H+ISO_H*4)continue;
      ...
      if(t===WALL){
        ...
        if(blk){
          ...
          // HP label
          const {sx,sy}=wp2s((c+.5)*ISO_W,(r+.5)*ISO_W);
...
function drawBullet(b){
  ...
  // trail
  const tx=sx-b.dx*1.5*(1), ty=sy-(b.dy+b.dx)*.5*(ISO_H/(2*ISO_W))*1.5;
