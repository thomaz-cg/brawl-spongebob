// game.js v7 - Classic 2D top-down view
...
// ═══ AUDIO ══════════════════════════════════════════════════
let audioCtx=null,masterGain=null,musicGain=null,menuMusicGain=null;
function getAudio(){
  if(!audioCtx){
    ...
    menuMusicGain=audioCtx.createGain();menuMusicGain.gain.value=1;menuMusicGain.connect(audioCtx.destination);
  }
  return audioCtx;
}
...
function playShoot(charKey){
  ...
  try{
    ...
    const cfg=cfgs[charKey]||cfgs.spongebob;
    o.type=cfg.type;
...
// Shared melody notes (SpongeBob theme)
const MELODY_NOTES=[
...
function playMelodyOnce(startT,gainNode,vol){
  const ac=getAudio();let t=startT;
  for(let i=0;i<MELODY_NOTES.length;i+=3){
    const freq=MELODY_NOTES[i],gap=MELODY_NOTES[i+1],dur=MELODY_NOTES[i+2];
    if(freq>0){
      const o=ac.createOscillator(),g=ac.createGain(),dist=ac.createWaveShaper();
      ...
      g.gain.linearRampToValueAtTime((vol||.1)*.8,t+dur*.6);g.gain.linearRampToValueAtTime(0,t+dur);
      o.connect(dist);dist.connect(g);g.connect(gainNode);
...
// Game music (plays during battle)
function startGameMusic(){
  try{
    const ac=getAudio();
    if(ac._gameMusicStarted)return;
    ac._gameMusicStarted=true;
    musicGain.gain.value=musicEnabled?1:0;
    let nextT=ac.currentTime+.1;
    function loop(){
      if(!gRunning){ac._gameMusicStarted=false;return;}
      nextT=playMelodyOnce(nextT,musicGain,.1);
      setTimeout(loop,(nextT-ac.currentTime-1)*1000);
    }
    loop();
  }catch(e){}
}

// Menu music (plays on overlay, slightly different volume/feel)
function startMenuMusic(){
  try{
    const ac=getAudio();
    if(ac._menuMusicStarted)return;
    ac._menuMusicStarted=true;
    menuMusicGain.gain.value=musicEnabled?0.7:0;
    let nextT=ac.currentTime+.2;
    function loop(){
      if(!menuMusicRunning){ac._menuMusicStarted=false;return;}
      nextT=playMelodyOnce(nextT,menuMusicGain,.07);
      setTimeout(loop,(nextT-ac.currentTime-1)*1000);
    }
    loop();
  }catch(e){}
...
function stopMenuMusic(){
  ...
  if(menuMusicGain)menuMusicGain.gain.setTargetAtTime(0,getAudio().currentTime,.3);
}
...
function drawSpongebob(ctx,x,y,dir,team){
  ...
  ctx.fillStyle='#7a3b10';ctx.fillRect(x-10,y+8,20,9);ctx.fillStyle='#5c2c0a';ctx.fillRect(x-10,y+8,20,2);ctx.fillStyle='#a04c15';ctx.fillRect(x-3,y+8,6,9);
  const bg=ctx.createLinearGradient(x-10,y-12,x+10,y+8);bg.addColorStop(0,'#ffe840');bg.addColorStop(.4,'#f0d020');bg.addColorStop(1,'#c8aa10');
  ctx.fillStyle=bg;ctx.fillRect(x-10,y-10,20,20);
  ...
  const hg=ctx.createLinearGradient(x-10,y-26,x+10,y-10);hg.addColorStop(0,'#ffe840');hg.addColorStop(.5,'#f0d020');hg.addColorStop(1,'#c8aa10');
  ctx.fillStyle=hg;ctx.fillRect(x-10,y-26,20,16);
  ...
  ctx.fillStyle='#2244cc';ctx.beginPath();ctx.arc(x-4+f*1.2,y-19.5,2.8,0,Math.PI*2);ctx.fill();ctx.beginPath();ctx.arc(x+4+f*1.2,y-19.5,2.8,0,Math.PI*2);ctx.fill();
...
function drawPatrick(ctx,x,y,dir,team){
  ...
  const bg=ctx.createRadialGradient(x-4,y-4,2,x,y,15);bg.addColorStop(0,'#ffaacc');bg.addColorStop(.5,'#ff80a0');bg.addColorStop(1,'#cc5580');
  ctx.fillStyle=bg;ctx.beginPath();ctx.arc(x,y,15,0,Math.PI*2);ctx.fill();
  ctx.fillStyle='rgba(255,220,240,0.4)';ctx.beginPath();ctx.ellipse(x-5,y-6,7,5,-.3,0,Math.PI*2);ctx.fill();
...
function drawSquidward(ctx,x,y,dir,team){
  ...
  ctx.strokeStyle='#4aafaf';ctx.lineWidth=3.5;ctx.lineCap='round';
  [[-8,-1.5],[-3,-.5],[3,.5],[8,1.5]].forEach(([ox,tw])=>{ctx.beginPath();ctx.moveTo(x+ox,y+14);ctx.bezierCurveTo(x+ox*1.5,y+20,x+ox*1.8+tw*3,y+24,x+ox*2.2,y+30);ctx.stroke();});
  const bg=ctx.createLinearGradient(x-11,y-5,x+11,y+14);bg.addColorStop(0,'#7ed9d9');bg.addColorStop(1,'#4aafaf');
  ctx.fillStyle=bg;ctx.beginPath();ctx.ellipse(x,y+5,11,14,0,0,Math.PI*2);ctx.fill();
  ...
  const hg=ctx.createLinearGradient(x-10,y-34,x+10,y-10);hg.addColorStop(0,'#8ee0e0');hg.addColorStop(1,'#5cbfbf');
  ctx.fillStyle=hg;ctx.beginPath();ctx.ellipse(x,y-18,10,15,0,0,Math.PI*2);ctx.fill();
...
function drawSandy(ctx,x,y,dir,team){
  ...
  ctx.beginPath();ctx.moveTo(x+f*9,y+4);ctx.bezierCurveTo(x+f*20,y+2,x+f*25,y-4,x+f*22,y-14);ctx.stroke();
  const bg=ctx.createRadialGradient(x-3,y-3,1,x,y,12);bg.addColorStop(0,'#f0bc50');bg.addColorStop(1,'#c88820');
  ctx.fillStyle=bg;ctx.beginPath();ctx.arc(x,y,12,0,Math.PI*2);ctx.fill();
...
function drawGary(ctx,x,y,dir,team){
  ...
  const sg=ctx.createRadialGradient(x-3,y-4,2,x,y,13);sg.addColorStop(0,'#e09850');sg.addColorStop(.5,'#cc8840');sg.addColorStop(1,'#aa6620');
  ctx.fillStyle=sg;ctx.beginPath();ctx.arc(x,y,13,0,Math.PI*2);ctx.fill();
  ...
  const cg=ctx.createRadialGradient(x-2,y-2,1,x,y,6);cg.addColorStop(0,'#ffcc60');cg.addColorStop(1,'#e09830');
  ctx.fillStyle=cg;ctx.beginPath();ctx.arc(x,y,5.5,0,Math.PI*2);ctx.fill();
...
let keys={};
let soundEnabled=true,musicEnabled=true,menuMusicRunning=false;
...
document.querySelectorAll('.cc').forEach(c=>{
  c.addEventListener('click',function(){
    ...
    showCharQuote(selChar);
  });
});
document.getElementById('sbtn').addEventListener('click',startGame);

// Menu music mute toggle (called from HTML button)
function toggleMenuMusic(){
  musicEnabled=!musicEnabled;
  if(menuMusicGain)menuMusicGain.gain.setTargetAtTime(musicEnabled?.7:0,getAudio().currentTime,.15);
  if(musicGain)musicGain.gain.setTargetAtTime(musicEnabled?1:0,getAudio().currentTime,.15);
  const btn=document.getElementById('muteBtn');
  if(btn){btn.textContent=musicEnabled?'🔊':'🔇';btn.title=musicEnabled?'Mutar música':'Desmutar música';}
}

// Character quotes and voice sounds
const CHAR_QUOTES={
  spongebob:{quote:'Estou pronto! 🧽',color:'#ffe840'},
  patrick:  {quote:'Ei, qual é mesmo o meu nome? ⭐',color:'#ff90b8'},
  squidward:{quote:'Que desperdício do meu talento... 🐙',color:'#7ed9d9'},
  sandy:    {quote:'Yee-haw! Vamos pegar uns jacarés! 🤠',color:'#f0bc50'},
  gary:     {quote:'Miau... 🐌',color:'#cc8840'},
};

function playCharVoice(charKey){
  try{
    const ac=getAudio();
    const voices={
      spongebob:()=>{
        // SpongeBob laugh: rapid high ascending notes
        [0,.07,.14,.21,.28,.35].forEach((t,i)=>{
          const o=ac.createOscillator(),g=ac.createGain();
          o.type='sine';o.frequency.value=600+i*120;
          g.gain.setValueAtTime(0,ac.currentTime+t);g.gain.linearRampToValueAtTime(.22,ac.currentTime+t+.03);g.gain.linearRampToValueAtTime(0,ac.currentTime+t+.06);
          o.connect(g);g.connect(masterGain);o.start(ac.currentTime+t);o.stop(ac.currentTime+t+.08);
        });
      },
      patrick:()=>{
        // Patrick: slow dumb descending
        [0,.18,.36].forEach((t,i)=>{
          const o=ac.createOscillator(),g=ac.createGain();
          o.type='sawtooth';o.frequency.setValueAtTime(220-i*30,ac.currentTime+t);
          o.frequency.exponentialRampToValueAtTime(120-i*20,ac.currentTime+t+.15);
          g.gain.setValueAtTime(.2,ac.currentTime+t);g.gain.linearRampToValueAtTime(0,ac.currentTime+t+.16);
          o.connect(g);g.connect(masterGain);o.start(ac.currentTime+t);o.stop(ac.currentTime+t+.18);
        });
      },
      squidward:()=>{
        // Squidward: nasally clarinet-like
        const o=ac.createOscillator(),g=ac.createGain(),dist=ac.createWaveShaper();
        const cv=new Float32Array(256);for(let j=0;j<256;j++){const x=j*2/256-1;cv[j]=Math.tanh(x*3)*.5;}
        dist.curve=cv;o.type='triangle';
        o.frequency.setValueAtTime(350,ac.currentTime);
        o.frequency.setValueAtTime(280,ac.currentTime+.15);
        o.frequency.setValueAtTime(310,ac.currentTime+.3);
        g.gain.setValueAtTime(.18,ac.currentTime);g.gain.linearRampToValueAtTime(0,ac.currentTime+.5);
        o.connect(dist);dist.connect(g);g.connect(masterGain);o.start();o.stop(ac.currentTime+.55);
      },
      sandy:()=>{
        // Sandy: cheerful yee-haw
        [[440,.1],[550,.08],[660,.12],[550,.08],[440,.1]].forEach(([freq,dur],i)=>{
          const t=i*.12;
          const o=ac.createOscillator(),g=ac.createGain();
          o.type='square';o.frequency.value=freq;
          g.gain.setValueAtTime(0,ac.currentTime+t);g.gain.linearRampToValueAtTime(.12,ac.currentTime+t+.02);g.gain.linearRampToValueAtTime(0,ac.currentTime+t+dur);
          o.connect(g);g.connect(masterGain);o.start(ac.currentTime+t);o.stop(ac.currentTime+t+dur+.02);
        });
      },
      gary:()=>{
        // Gary: meow - soft mew
        ...
        o.frequency.linearRampToValueAtTime(600,ac.currentTime+.1);
        o.frequency.linearRampToValueAtTime(350,ac.currentTime+.3);
        g.gain.setValueAtTime(0,ac.currentTime);g.gain.linearRampToValueAtTime(.25,ac.currentTime+.05);
        g.gain.linearRampToValueAtTime(0,ac.currentTime+.32);
        o.connect(g);g.connect(masterGain);o.start();o.stop(ac.currentTime+.35);
      },
    };
    if(voices[charKey])voices[charKey]();
  }catch(e){}
}

function showCharQuote(charKey){
  let el=document.getElementById('char-quote');
  if(!el){
    el=document.createElement('div');el.id='char-quote';
    el.style.cssText='position:absolute;bottom:90px;left:50%;transform:translateX(-50%);'+
      'background:rgba(0,0,0,0.75);border-radius:12px;padding:8px 18px;'+
      'font-family:Fredoka One,cursive;font-size:15px;text-align:center;'+
      'pointer-events:none;transition:opacity .3s;white-space:nowrap;'+
      'border:1.5px solid rgba(255,255,255,0.15);z-index:200;';
    document.getElementById('obox').style.position='relative';
    document.getElementById('obox').appendChild(el);
  }
  const q=CHAR_QUOTES[charKey];
  el.style.color=q.color;el.textContent='"'+q.quote+'"';
  el.style.opacity='1';
  clearTimeout(el._t);
  el._t=setTimeout(()=>{el.style.opacity='0';},2200);
}
function buildMap(){
  MC=Math.max(17,Math.floor((W-20)/T));MR=Math.max(11,Math.floor((H-60)/T));
...
function startGame(){
  ...
  // reset sbtn spans for next time overlay opens
  document.getElementById('sbtn-text').textContent='BATALHAR!';
  document.getElementById('sbtn-icon').textContent='⚔️';
...
function quitGame(){
  ...
  if(audioCtx)audioCtx._gameMusicStarted=false;
  // Hide pause menu
  ...
  document.getElementById('sbtn-text').textContent='BATALHAR!';
  document.getElementById('sbtn-icon').textContent='⚔️';
  document.getElementById('otitle').textContent='🌟 Bob Stars';
  document.getElementById('osub').textContent='Setas = mover e mirar · ESPAÇO = atirar!';
  // Show main menu
  document.getElementById('overlay').style.display='flex';
  // Restart ocean background animation (stopped while game was running)
  if(typeof window.restartOceanAnim==='function') window.restartOceanAnim();
  // Restart menu music
...
function getAiTarget(p){
  ...
  const visible=enemies.filter(e=>{
    ...
    return tileAt(e.x,e.y)===BUSH?dist<BUSH_SIGHT:dist<SIGHT;
  });
...
function aiStep(p){
  ...
  if(!t){
    const cx=OX+MC*T/2,cy=OY+MR*T/2;
  ...
  p.facing=t.x>p.x?1:-1;p.aimDx=(t.x-p.x)/dist;p.aimDy=(t.y-p.y)/dist;
  shootP(p);
}
function hitBlock(bx,by){
  const c=~~((bx-OX)/T),r=~~((by-OY)/T);
  const blk=blocks.find(b=>b.r===r&&b.c===c);
  if(blk){blk.hp--;spawnChips(bx,by);if(blk.hp<=0){tiles[r][c]=0;blocks.splice(blocks.indexOf(blk),1);spawnChips(bx,by,true);}return true;}
  return false;
}
function updBullets(){
  ...
  for(let r=0;r<MR;r++)for(let c=0;c<MC;c++){
    ...
    if(t===WALL){
      ...
      if(blk){
        ...
        const grd=ctx.createLinearGradient(x,y,x+T,y+T);grd.addColorStop(0,topCol);grd.addColorStop(1,midCol);
        ...
        if(blk.hp<2){ctx.strokeStyle='rgba(0,0,0,0.6)';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(x+T-5,y+5);ctx.lineTo(x+5,y+T-5);ctx.stroke();}
        ...
        const wgrd=ctx.createLinearGradient(x,y,x+T,y+T);wgrd.addColorStop(0,'#1a3a6a');wgrd.addColorStop(.4,'#0d2448');wgrd.addColorStop(1,'#081830');
      ...
      const bgrd=ctx.createLinearGradient(x,y,x+T,y+T);bgrd.addColorStop(0,'#0a3520');bgrd.addColorStop(1,'#051810');
      ctx.fillStyle=bgrd;ctx.fillRect(x,y,T,T);ctx.fillStyle='#0e4828';ctx.fillRect(x+1,y+1,T-2,T-2);
...
function drawPlayer(p){
  ...
  const inBush=tileAt(p.x,p.y)===BUSH,isMe=p===players[0],isAlly=p.team==='blue'&&!isMe;
  const bushAlpha=isMe?.45:isAlly?.4:.18;
  ...
  ctx.fillStyle='rgba(0,0,0,0.7)';ctx.fillRect(bx-1,by-1,bw+2,bh+2);ctx.fillStyle='#1a1a1a';ctx.fillRect(bx,by,bw,bh);
...
function checkEnd(){
  ...
  setTimeout(()=>{
    ...
    document.getElementById('sbtn-text').textContent='Jogar Novamente';
    document.getElementById('sbtn-icon').textContent='🔄';
    document.getElementById('menuBtn').style.display='none';
    document.getElementById('overlay').style.display='flex';
    if(typeof window.restartOceanAnim==='function') window.restartOceanAnim();
    menuMusicRunning=true;startMenuMusic();
  },900);
}
...
function loop(){
  ...
  for(const b of bullets){
    ...
    ctx.globalAlpha=1;ctx.fillStyle=b.color;ctx.beginPath();ctx.arc(b.x,b.y,b.r,0,Math.PI*2);ctx.fill();
...
// Start menu music when page loads (triggered on first user interaction)
window.addEventListener('click',function startOnClick(){
  menuMusicRunning=true;
  startMenuMusic();
