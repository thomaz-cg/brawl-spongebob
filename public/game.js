...
function startMusic(){
  try{
    ...
    function loop(){
      ...
      nextT=end;
      setTimeout(loop,(end-ac.currentTime-1)*1000);
    }
    loop();
  }catch(e){}
}
...
const CHARS={
  spongebob:{speed:3.0,hp:300,dmg:20,bs:6.2,fr:26,bColor:'#00cfff',bR:4,r:15},
  patrick:{speed:1.7,hp:500,dmg:55,bs:4.0,fr:56,bColor:'#ff66aa',bR:6,r:16},
  squidward:{speed:2.4,hp:320,dmg:33,bs:8.8,fr:36,bColor:'#ccff44',bR:4,r:14},
  sandy:{speed:3.3,hp:280,dmg:22,bs:7.2,fr:28,bColor:'#ffcc00',bR:4,r:14},
  gary:{speed:1.3,hp:600,dmg:28,bs:3.5,fr:44,bColor:'#88ffaa',bR:5,r:15},
};
...
function shootP(s){
  ...
  bullets.push({x:s.x,y:s.y,dx:dx*s.bs,dy:dy*s.bs,team:s.team,dmg:s.dmg,color:d.bColor,r:d.bR,life:95,char:s.char});
}
function aiStep(p){
  if(!p.alive)return;
...
function drawPlayer(p){
  ...
  ctx.save();ctx.globalAlpha=(inBush&&!isMe)?.05:.2;
