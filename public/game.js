...
function getAiTarget(p){
  // Se o jogador (players[0]) estiver numa alga, os inimigos NÃO o enxergam
  // (a menos que já estejam perto o suficiente para "ouvir")
  const SIGHT=240, BUSH_SIGHT=60;
  const enemies=players.filter(e=>e.team!==p.team&&e.alive);
  if(!enemies.length)return null;

  // filtra inimigos que o bot consegue "ver"
  const visible=enemies.filter(e=>{
    const dist=Math.hypot(e.x-p.x,e.y-p.y);
    const eInBush=tileAt(e.x,e.y)===BUSH;
    // se o inimigo está numa alga, só vê se estiver muito perto
    if(eInBush)return dist<BUSH_SIGHT;
    return dist<SIGHT;
  });

  if(!visible.length){
    // ninguém visível: anda para o lado da tela do adversário devagar
    return null;
  }
  // ataca o inimigo visível mais próximo (sem priorizar o jogador)
...
function aiStep(p){
  ...
  if(!t){
    // sem alvo visível: patrulha em direção ao centro do mapa
