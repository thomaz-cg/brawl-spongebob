...
function toggleMusic(){
  ...
  if(musicEnabled&&gRunning){if(audioCtx)audioCtx._musicStarted=false;startMusic();}
}
...
function shootP(s){
  ...
  if(s!==players[0]){
    // bots atiram no inimigo mais próximo
    const enemies=players.filter(p=>p.team!==s.team&&p.alive);
    if(!enemies.length)return;
    // time vermelho: prioriza o jogador (players[0]) se estiver vivo
    let t;
...
function aiStep(p){
  ...
  if(!enemies.length)return;
  // time vermelho persegue o jogador (players[0]) enquanto ele estiver vivo
  let t;
  if(p.team==='red'&&players[0].alive){
    t=players[0];
  }else{
    // aliados e bots azuis pegam o inimigo mais próximo
